...
import 'package:sams_app/features/live_sessions/presentation/view/live_session_tap_view.dart/widget/web/web_meeting_card.dart';
...
class LiveSessionsWebLayout extends StatelessWidget {
    ...
    return BlocListener<MeetingCubit, MeetingState>(
      ...
      child: BlocBuilder<MeetingCubit, MeetingState>(
        ...
        builder: (context, state) {
          Widget sliverItem = const SliverToBoxAdapter(
            child: SizedBox.shrink(),
          );

          if (state is FetchMeetingsLoading) {
            sliverItem = const SliverFillRemaining(
            ...
            if (itemCount == 0) {
              sliverItem = const SliverFillRemaining(
                child: Center(child: Text('No Live Sessions found')),
              );
            } else {
              sliverItem = SliverPadding(
                  ...
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      return WebMeetingCard(
                        ...
                        onEnd: () {
                          context.read<MeetingCubit>().endMeeting(
                            meetingId: state.meetings[meetingIndex].id,
                            courseId: courseId,
                          );
                        },
                        onDelete: () {
                          context.read<MeetingCubit>().deleteMeeting(
                            meetingId: state.meetings[meetingIndex].id,
                            courseId: courseId,
                          );
                        },
